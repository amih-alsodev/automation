...
from utils.api.api_base import get_global_site_settings
...
def get_site_params_from_list(site_data, site_name):
    """
...
def save_site_params_to_file(playwright: Playwright, site_list):
    """
    ...
    for site in site_list:
        site_data = get_global_site_settings(playwright, site["token"])
        site_params = get_site_params_from_list(site_data, site["project"])
...
def get_selected_site_param_from_file(name):
    """
...
def get_payload_from_file(file_name, list_title):
    with open("../data/"+file_name+"") as f:
        file_data = json.load(f)
        required_list = file_data[list_title]
    return required_list